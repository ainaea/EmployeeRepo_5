...
namespace EmployeeManagement.Controllers
{
    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> ManageUserClaims(UserClaimsViewModel model)
        {
            ...
            if (!result.Succeeded)
            {
                ModelState.AddModelError(string.Empty, "Cannot remove user's existing claims");
                return View(model);
            }
            result = await userManager.AddClaimsAsync(user, model.Claims.Select(u => new Claim (u.ClaimType, u.IsSelected? "true": "false")));
            if (!result.Succeeded)
            {
        ...
        public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            else
            {
                ...
                catch (DbUpdateException ex)
                {
                    ...
                    ViewBag.ErrorMessage = $"{role.Name} role cannot be deleted as there are users in this role. If you want to delete this role, please remove the users from the role then try to delete";
                    return View("Error");
                }
                catch(Exception ex)
                {
        ...
        public async Task<IActionResult> EditRole(string id)
        {
            ...
            //foreach (var user in userManager.Users)       //throws exception InvalidOperationException: There is already an open DataReader associated with this Connection which must be closed first. 
        ...
        public async Task<IActionResult> ManageUserRoles(List<UserRolesViewModel> model, string userId)
        {
            ...
            result = await userManager.AddToRolesAsync(user, model.Where( u => u.IsSelected).Select( u=> u.RoleName));
            if (!result.Succeeded)
            {
